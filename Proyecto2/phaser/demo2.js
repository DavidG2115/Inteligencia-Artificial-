...
function update() {
    ...
    if( modoAuto ==false  && bala.position.x > 0 ){
        ...
        datosEntrenamiento.push({
            ...
            'output': [estatusAire, estatuSuelo]  // ← ¡Aquí ya correcto!
        });
...
function descargarDatasetCSV() {
    let csvContent = "data:text/csv;charset=utf-8,";
    
    // Agregamos cabeceras
    csvContent += "despBala,velocidadBala,estatusAire,estatuSuelo\n";
    
    // Agregamos datos
    datosEntrenamiento.forEach(function(fila) {
        let despBala = fila.input[0];
        let velocidadBala = fila.input[1];
        let estatusAire = fila.output[0];
        let estatuSuelo = fila.output[1];
        
        let filaCSV = `${despBala},${velocidadBala},${estatusAire},${estatuSuelo}\n`;
        csvContent += filaCSV;
    });
